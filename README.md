...
3- إضافة (Api Gateway) : يسهل إدارة جميع الطلبات من المستخدمين، ويوفر طبقة أمن(Authentication & Authorization) وتوجيه للطلبات الي الوحدات المصغرة الخاصة بها
(microservices)
...
6- يمكن إضافة (redis caching) مع نظام البحث و الاكتشاف لتقليل الضغط على ال (elastic search) للبيانات التي يتم استرجاعها بشكل متكرر و تخزينها ذاكرة عالية السرعة (redis)
...
8- يمكن إضافة (Service Discovery/Registry) بحيث تقوم الخدمات
...
10- يمكن إضافة ملف(docker-compose.yml) لتسهيل التشغيل على البيئات المختلفة (multiple
environments)
